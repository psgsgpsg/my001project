...
public:
   ...
   virtual BOOL SetScalingMode(ScalingMode scalingMode, BOOL bKeepAspectRatio= FALSE);
   virtual BOOL SetScalingMode(float zoomHorz, float zoomVert);
   virtual BOOL SetScalingMode(const SapViewScaleParams& srcParams, const SapViewScaleParams& dstParams);
...
protected:
   ...
   BOOL   m_NextIndex;        // Index of the next buffer to display
