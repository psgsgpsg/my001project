										...
										const size_t		sizeLimit)
{
	...
	FBlobSetImage				(blob, image, 0);
	FBlobSetObjectFeatureRange	(blob, 0, 128);
	FBlobSetLimitArea			(blob, sizeLimit, -1);
	FBlobExec					(blob);
									...
									const int		imgWidth)
{
	...
	for( i = 0; i < rectCount; ++i)
	{
		bool	isEmpty = (m_mode == REMAINMODE::TRANSMODE)?
