...
public:
	...
	SapAcqToBufThruDsp(SapAcquisition *pAcq, SapDsp *pDsp, SapBuffer *pBuf, SapXferCallback pCallback = NULL, void *pContext = NULL)
		: SapTransfer(pCallback, pContext)
	{
		AddPair(SAPCLASS_REF(SapXferPair,SapXferPair(pAcq, pDsp, FALSE)));
		AddPair(SAPCLASS_REF(SapXferPair,SapXferPair(pDsp, pBuf)));
	}
};
...
public:
	...
	{
		AddPair(SAPCLASS_REF(SapXferPair,SapXferPair(pAcq, pDsp, FALSE)));
		AddPair(SAPCLASS_REF(SapXferPair,SapXferPair(pDsp, pCab, FALSE)));
	}
};
