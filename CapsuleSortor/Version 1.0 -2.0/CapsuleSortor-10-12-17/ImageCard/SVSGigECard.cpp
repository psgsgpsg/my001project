...
CameraContainerClient_handle	SVSGigECard::m_hClient = SVGigE_NO_CLIENT;
...
bool SVSGigECard::Open(const char * camTag)
{
	...
	if (SVGigE_SUCCESS != Camera_setAcquisitionModeAndStart(m_hCamera, ACQUISITION_MODE_EXT_TRIGGER_INT_EXPOSURE, false))
	{
		return false;
	}
...
bool	SVSGigECard::SetROI(	size_t startX,
						   ...
						   size_t height)
{
	if (SVGigE_NO_STREAMING_CHANNEL== m_hChannel)
	{
		return false;
	}
	//得到当前的获取状态
	ACQUISITION_CONTROL curState;
	if (SVGigE_SUCCESS != Camera_getAcquisitionControl(m_hCamera, &curState))
	{
		return false;
	}
	//得到当前的获取模式
	ACQUISITION_MODE curMode;
	if (SVGigE_SUCCESS != Camera_getAcquisitionMode(m_hCamera, &curMode))
	{
		return false;
	}
	
	Camera_setAcquisitionControl(m_hCamera, ACQUISITION_CONTROL_STOP);
	//删除数据通道
	if (SVGigE_SUCCESS != StreamingChannel_delete(m_hChannel))
	{
		return false;
	...
	//重新创建通道
	if (SVGigE_SUCCESS != StreamingChannel_create(&m_hChannel,
		m_hClient,
		m_hCamera,
		0,
		ImageCallback,
	...
	if (SVGigE_SUCCESS != Camera_setAcquisitionModeAndStart(m_hCamera, curMode, false))
	{
		return true;
	}
	//回复到原来的相机状态
	if (ACQUISITION_CONTROL_START == curState)
	{
		Camera_setAcquisitionControl(m_hCamera, ACQUISITION_CONTROL_START);
	}
	return true;
}
...
{	
	try
	{
		...
		if (Data != NULL)
		{
			...
			pCard->m_stampInterval = pCard->m_timeCounter.msec();
			pCard->m_timeCounter.Reset();
			pCard->m_pixelMemBase = Data;
			/*
			if (pCard->m_stampInterval <pCard->m_oldInterval)
			{
				Sleep(25);
			}*/
			pCard->m_oldInterval = pCard->m_stampInterval;
			pCard->m_imageSignal.PulseEvent();
		}
	}
	catch(...)
	{
