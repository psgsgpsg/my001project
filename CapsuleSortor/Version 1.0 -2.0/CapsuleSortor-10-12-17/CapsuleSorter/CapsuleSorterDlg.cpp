...
void CapsuleSorterDlg::OnClearCount() 
{
	...
	CtrlCardComm::TheComm().LightCtrl(CtrlCardComm::CLOSEALL, true);
	UpdateData(FALSE);
}
...
void	CapsuleSorterDlg::FirstCamFunc	(void *param)
{
	...
	{		
		...
		try
		{
			...
			pThis->IsPackageFull(pThis->m_firstProcess.AllCount(), pThis->m_firstProcess.BadCount());
...
void  CapsuleSorterDlg::IsPackageFull(int totalNum, int badNum)
{
	static int PKGCap = ConfigInfo::TheConfigInfo().PKGCapability();
	static bool enterFlag = false;
	if (abs(totalNum - badNum) >= PKGCap )
	{
		if (!enterFlag) 
		{
			CtrlCardComm::TheComm().LightCtrl(CtrlCardComm::RED, true);
			enterFlag = true;
		}		
	}	
	else
