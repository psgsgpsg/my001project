...
TBeacon::~TBeacon()
{
	CtrlCardComm::TheComm().LightCtrl(CtrlCardComm::CLOSEALL, false);
}
...
void	TBeacon::TurnGreen		(bool shining)
{
	if(m_greenShining != shining )
	{
		...
		if (m_greenShining)	
		{
			...
			m_yellowShining	= false;
			CtrlCardComm::TheComm().LightCtrl	(CtrlCardComm::CLOSEALL, false);
			CtrlCardComm::TheComm().LightCtrl	(CtrlCardComm::GREEN,		true);
		}
	}
}
...
{	
	if(m_yellowShining != shining )
	{
		...
		if (m_yellowShining )
		{
			...
			m_redShining	= false;
			CtrlCardComm::TheComm().LightCtrl	(CtrlCardComm::CLOSEALL, false);
			CtrlCardComm::TheComm().LightCtrl	(CtrlCardComm::YELLOW,		true);
		}
	}
}
