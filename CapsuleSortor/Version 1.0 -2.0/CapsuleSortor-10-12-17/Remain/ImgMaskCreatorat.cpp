...
void ImgMaskCreator::DrawArc(TPoint2D<long> point0, TPoint2D<long> point1, size_t radius)
{
	...
	if( (point0.x()+point1.x())/2 < xCenter)
	{
	...
	if(arg1 < 0) arg1 = 360*PIdiv180 + arg1;
	if(arg2 < 0) arg2 = 360*PIdiv180 + arg2;
	...
	{
		TPoint2D<long> insert1 = point0.RotateRoundPoint(rotCenter,  rotAngle);
...
void ImgMaskCreator::ScanFill(const TRect2D<int>& range, PelGray8 fillVal)
{
	...
	{
		PelGray8 *pDataL = iterBegin.Base();
		PelGray8 *pDataR = pDataL+1;
		x = 0;
...
std::vector< TRect2D<int> > ImgMaskCreator::GetFillRectV(size_t index)
{
	...
	TRect2D<int>	window ( TPoint2D<int>((xPos.first+xPos.last)/2, 0), 1, height);
