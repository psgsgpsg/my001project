...
unsigned int CapsuleProc::TransProcess( WORKMODE mode )
{
	...
	m_transRemain.RemainCapsule(m_rawImage, dim, m_profileImage, m_remainParam, TTransRemain::TRANSMODE);
...
{
	if (static_cast<size_t>(abs(static_cast<int>(realSize - stdSize1))<tolerance||abs(static_cast<int>(realSize - stdSize2)))<tolerance)
	{
		return true;
	}
	return false;
