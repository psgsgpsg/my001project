...
void CControlBoardSetting::OnSetTimeorder() 
{
	...
	theComm.PulseSet(atoi(m_camIndex), static_cast<USHORT>(m_camDelay), static_cast<USHORT>(m_camHold), static_cast<USHORT>(m_lightDelay), static_cast<USHORT>(m_lightHold));
	Sleep(100);
	theComm.SenserCfg(atoi(m_sensorIndex), static_cast<USHORT>(m_shieldTime));
...
void CControlBoardSetting::OnSetComplex() 
{
	...
	theComm.ValveCfg(static_cast<UCHAR>(atoi(m_valveIndex)), static_cast<USHORT>(m_valveDelay), static_cast<USHORT>(m_valveHold) );
...
void CControlBoardSetting::OnSetSpace() 
{
	...
	CtrlCardComm &theComm = CtrlCardComm::TheComm();
	theComm.CamValveDistance(static_cast<UCHAR>(m_camInter_selection1.GetCurSel() + 1), static_cast<USHORT>(m_camValveDistance1));
	theComm.CamValveDistance(static_cast<UCHAR>(m_camInter_selection2.GetCurSel() + 1), static_cast<USHORT>(m_camValveDistance2));
...
void CControlBoardSetting::OnSetOther() 
{
	...
	CtrlCardComm& theComm = CtrlCardComm::TheComm();
	theComm.CapsuleInterval	(static_cast<USHORT>(m_capsuleInterval));
	Sleep(100);
	theComm.AxisRadius		(static_cast<USHORT>(m_axisRadius));
	Sleep(100);
	theComm.RotCoderCfg		(static_cast<USHORT>(m_codePrecision), static_cast<USHORT>(m_codeAdjust));
	Sleep(100);
	theComm.PulseInterval	(static_cast<USHORT>(m_pulseInterval));
