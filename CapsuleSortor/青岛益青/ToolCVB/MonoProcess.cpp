...
unsigned int MonoProcess::ComProcess( WORKMODE mode)
{
	...
	TImgDim dim = Dimension();
	m_transRemain.RemainCapsule(	m_rawImage, 
									dim,
									m_profileImage,
									m_remainParam, 
									TTransRemain::REMAINMODE::OPAQUEMODE);
									...
									size_t downEdge)const
{
	...
	static float	lengthMean = height + downEdge - upEdge;
	static float	changeMean = 15.0f;
	float			newLength  = height + downEdge - upEdge;
