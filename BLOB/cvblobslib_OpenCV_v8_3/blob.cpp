...
CvBox2D CBlob::GetEllipse()
{
	...
	m_ellipse.center.x = static_cast<float>(u10);
	m_ellipse.center.y = static_cast<float>(u01);
	
	temp = u20 + u02 + delta;
	if( temp > 0 )
	{
		m_ellipse.size.width = static_cast<float>(sqrt( 2*(u20 + u02 + delta )));
	}	
	else
	{
		m_ellipse.size.width = 0;
	...
	if( temp > 0 )
	{
		m_ellipse.size.height = static_cast<float>(sqrt( 2*(u20 + u02 - delta ) ));
	}
	else
	{
		m_ellipse.size.height = 0;
	...
	if( num != 0 && den  != 00 )
	{
		m_ellipse.angle = static_cast<float>(180.0 + (180.0 / CV_PI) * atan( num / den ));
	}
	else
	{
		m_ellipse.angle = 0;
